...
**The mirror is deleted, not lost.** The `html-prototype` branch is the last commit
that carried it, and `scripts/revert-to-html.sh` puts it back without touching the
port's history. `docs/revert-to-html.md` is the whole procedure,
