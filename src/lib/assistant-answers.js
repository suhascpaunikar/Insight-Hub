/* ==========================================================================
   ...
   ========================================================================== */
import { count, percent, ratingText } from './format.js';
...
function campaignAnswer(context) {
  ...
  // The stated objective is the campaign's own words, so it is quoted, not
  // summarised — and it is worth quoting whichever kind the campaign is.
  const objective = (campaign.objective || '').trim();
  if (objective) text += ` It was set up to: “${objective}”`;
  const fallback = announce ? ['engagement', 'conversion'] : ['drivers', 'delivery'];
  return { text, followUps: objective ? ['objective', fallback[0]] : fallback };
}
  ...
  if (!source.objective || !source.objective.trim()) {
    return {
      text:
        ...
        `its trigger, its audience, ${isAnnounce(context) ? 'its channel' : 'its rating element'}` +
        ' — but not what it is for, because nothing ' +
