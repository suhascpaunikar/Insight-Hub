...
export const store = {
  ...
  /**
   * Write through to localStorage, then tell React.
   *
   * Every mutator below ends in `this.save()`, which makes this the one
   * notification point for the whole store — the vanilla build re-rendered by
   * hand at each call site instead. `version` is the snapshot
   * `useSyncExternalStore` compares: `state` is mutated in place, so its
   * identity cannot be the signal.
   */
  save() {
