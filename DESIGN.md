...
# Dark product surface — InsightHub adaptation

> Added for this repository. The generated sections above describe Supabase's **marketing**
> track, which commits to a white canvas. InsightHub is a **product console**, so it runs on
> Supabase's dashboard (Studio) dark scale instead. Everything else — the emerald primary, the
> 6px button radius, near-black-on-green button type, mono for figures — carries over unchanged.

## Dark scale

Supabase's dashboard is built on a numbered neutral scale rather than an ad-hoc grey ladder.
Each step has one job, and nothing in this prototype invents a neutral outside it.

| Token | Value | Role |
|---|---|---|
| `--background-200` | `#121212` | App canvas — the deepest surface, behind everything |
| `--background-100` | `#1c1c1c` | Default panel / card fill, nav rail, bars |
| `--surface-100` | `#232323` | Raised surface: table header, popover, inset well |
| `--surface-200` | `#282828` | Hover fill, selected row, secondary button |
| `--surface-300` | `#323232` | Pressed fill, active chip |
| `--border-muted` | `#232323` | Hairline where separation is barely needed |
| `--border-default` | `#2e2e2e` | Standard 1px border on cards, inputs, tables |
| `--border-strong` | `#3e3e3e` | Emphasised border, focus outline base |
| `--border-overlay` | `#4d4d4d` | Dialog and popover edges lifted off the page |
| `--foreground-muted` | `#707070` | Disabled type, placeholder |
| `--foreground-lighter` | `#898989` | Tertiary type, table meta |
| `--foreground-light` | `#b4b4b4` | Secondary type, helper copy, labels |
| `--foreground-default` | `#ededed` | Primary type |

`#ffffff` is never used as a text colour; `#ededed` is the top of the ramp, the exact inverse of
the marketing rule that `#171717` is the bottom.

## Brand on dark

- `--brand-default` `#3ecf8e` — the single filled CTA per view, the Live status dot, the top of
  the rating ramp. Scarce, as on the marketing surface.
- `--brand-600` `#24b47e` — pressed.
- `--brand-200` `rgba(62,207,142,0.12)` / `--brand-400` `rgba(62,207,142,0.35)` — the tinted
  fill + border pairing used for a green-toned status pill. Never a large area.
- Type on a filled emerald button stays near-black `#121212`, not white — the brand's
  idiosyncratic choice, preserved.

## Data palette

The console has to render charts, and the marketing brand has no data palette. Two rules keep
the additions from muddying it:

**The rating ramp** (FR-79 / FR-89) — five stops, normalised to whatever scale the campaign uses
(1–5 star, 1–5 NPS, or 1–10 NPS), so one colour means one thing on every surface:
...
| 2 | `#f76b15` | orange |
| 3 | `#ffb224` | amber |
| 4 | `#7cc47f` | light green |
| 5 · highest | `#3ecf8e` | brand emerald |

**The AI accent** (FR-91) — `#a78bfa`, violet. Reserved for machine inference: theme clusters,
AI suggestions, Intelligent A/B weighting. It appears nowhere in the rating ramp and nowhere in
the status palette, so a violet element is always a machine claim, never a measurement.

**Status** — Draft `#898989` · Scheduled `#3e9bff` · Live `#3ecf8e` · Paused `#ffb224` ·
Completed `#b4b4b4` · Stopped `#e5484d`. Every pill carries its label, so the colour is
reinforcement and never the sole carrier (FR-76).

## Typography on dark

Circular is proprietary, so the prototype runs the documented substitute: **Inter** for UI and
display, at the same weights and negative tracking. Figures — counts, ratings, percentages,
event names, campaign IDs — are set in the mono face so they align in columns (FR-77, FR-90).
The display sizes step down for console density: the console's largest heading is 22px, not
64px; marketing display tiers do not appear in the product.

## What the console adds

- **Focus ring** — `2px` `#3ecf8e` at 40% with a 1px background-coloured offset, on every
  interactive element. The marketing doc doesn't specify one; a console must.
